...
export default class extends Controller {
	...
	_initGlobe() {
		...
		svg
			...
			.attr('cy', H / 2)
			.attr('r', this.R)
			.attr('fill', 'url(#cai-og)');

		// Graticules violet subtil
		this._gratPath = svg
			.append('path')
			.datum(grat)
			.attr('stroke', 'rgba(155,89,255,0.12)')
		...
		sg.append('stop')
			.attr('offset', '0%')
			.attr('stop-color', 'rgba(192,132,252,0.2)');
		sg.append('stop')
			.attr('offset', '55%')
			.attr('stop-color', 'rgba(192,132,252,0)');
		svg
			.append('circle')
			.attr('cx', W / 2)
			.attr('cy', H / 2)
			.attr('r', this.R)
			.attr('fill', 'url(#cai-sg)')
		...
		ag.append('stop')
			.attr('offset', '82%')
			.attr('stop-color', 'rgba(155,89,255,0)');
		ag.append('stop')
			.attr('offset', '100%')
			.attr('stop-color', 'rgba(155,89,255,0.3)');
		this._atmo = svg
			.append('circle')
			.attr('cx', W / 2)
			.attr('cy', H / 2)
			.attr('r', this.R + 8)
			.attr('fill', 'url(#cai-ag)')
			.attr('pointer-events', 'none');
		...
		this._rim = svg
			...
			.attr('r', this.R)
			.attr('fill', 'none')
			.attr('stroke', 'rgba(155,89,255,0.5)')
			.attr('stroke-width', '2')
			.attr('pointer-events', 'none');
		...
		this._glow = svg
			...
			.attr('r', this.R + 16)
			.attr('fill', 'none')
			.attr('stroke', 'rgba(155,89,255,0.08)')
	...
	_redraw(grat) {
		...
		if (this.appState === 'speaking') {
			...
			this._rim
				.attr('stroke', `rgba(192,132,252,${p})`)
				.attr('stroke-width', 2 + Math.sin(this.animTime * 5));
			this._atmo.attr('r', this.R + 8 + Math.sin(this.animTime * 4) * 4);
			this._glow.attr(
				'stroke',
				`rgba(155,89,255,${0.1 + Math.sin(this.animTime * 4) * 0.06})`
			);
		} else if (this.appState === 'thinking') {
			const p = 0.4 + Math.sin(this.animTime * 9) * 0.2;
			this._rim
				.attr('stroke', `rgba(167,139,250,${p})`)
	...
	async send(event) {
		...
		try {
			...
			console.error('Census AI error:', error);
			this.setState('error');
		}
	}
